	...
	public static void main(String[] args) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[0].length; j++) {
                transposedMatrix[j][i] = matrix[i][j]; // Error: El índice i y j están invertidos (corregido)
            }
        }
        ...
        try {
            ...
            for (int i = 0; i < matrix.length; i++) {  // Error: índice fuera de límites (corregido)
                for (int j = 0; j < matrix[i].length; j++) {  // Error: índice fuera de límites (corregido)
                    sum += matrix[i][j];
                }
            }
